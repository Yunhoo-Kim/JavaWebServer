...
@URLAnnotation("master/dashboard/")
public class DashBoardView implements HttpHandler {
...
//            byte[] response = Collog.getInstance().getSlaveTable().toString().getBytes();
             ...
             */
            Iterator<JSONObject> iter = ((ArrayList<JSONObject>)json.get("dashboards")).iterator();
            while(iter.hasNext()){
                MasterMetaStorage.getInstance().addDashBoardData(iter.next());
            }
